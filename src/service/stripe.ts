...
export class StripeService {
   ...
   */
  constructor() {
    // Initialize with placeholder values, will be updated when secrets are loaded
    this.stripe = new Stripe('placeholder', {
      apiVersion: '2025-02-24.acacia',
    });
    this.clientId = 'placeholder';
    this.redirectUri = 'placeholder';
  }

  /**
   * Initializes the Stripe service with secrets from AWS Secrets Manager
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      const clientId =
        env.NODE_ENV === 'production'
          ? secrets.STRIPE_LIVE_CLIENT_ID
          : secrets.STRIPE_TEST_CLIENT_ID;

      const redirectUri = secrets.STRIPE_OAUTH_REDIRECT_URI;

      // Reinitialize Stripe client with actual secret key
      ...
      logger.info('Stripe service initialized with AWS Secrets Manager');
    } catch (error) {
      logger.error('Failed to initialize Stripe service with AWS Secrets Manager:', error);
      throw error;
    }
  }
   ...
   * @returns {Promise<Stripe.Event>} Constructed webhook event
   * @throws {Error} When event construction fails
   */
  public async constructWebhookEvent(payload: string, signature: string): Promise<Stripe.Event> {
    try {
      const webhookSecret = await secretsManager.getSecret(
        env.NODE_ENV === 'production' ? 'STRIPE_LIVE_WEBHOOK_SECRET' : 'STRIPE_TEST_WEBHOOK_SECRET',
      );
