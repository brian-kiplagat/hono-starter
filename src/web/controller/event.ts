...
export class EventController {
  ...
  public getEvents = async (c: Context) => {
    try {
      ...
      const { page, limit, search, date, eventType, sortOrder } = c.req.query();
      const query = {
  ...
  public getEvent = async (c: Context) => {
    try {
      ...
      if (!event.asset && event.event_type === 'prerecorded') {
        return serveBadRequest(c, ERRORS.ASSET_NOT_FOUND);
      }
      const [
        clicks,
        click_count,
        schedule_callback_count,
        upgrade_count,
        instant_callbacks,
        event_attended,
        event_not_attended,
        ...
        this.clickService.findByEventId(eventId),
        this.clickService.getClickCountByEventId(eventId),
        this.clickService.getClickCountByType(eventId, 'schedule_callback'),
        this.clickService.getClickCountByType(eventId, 'upgrade'),
        this.callbackService.getCallbacksByEventIdAndCallbackType(eventId, 'instant'),
        this.leadService.findLeadsByAttendedStatus(0, true, [eventId]),
        this.leadService.findLeadsByAttendedStatus(0, false, [eventId]),
        this.telemetryService.getLobbyTelemetryByEventId(eventId, false),
      ]);
      ...
      return c.json({
        ...
        lobby_telemetry,
        attendance_stats: {
          event_attended: event_attended.length,
          event_not_attended: event_not_attended.length,
        },
        click_analytics: {
          clicks,
          click_count,
  ...
  public createEvent = async (c: Context) => {
    try {
      ...
      // Get hostId and teamId from context to determine the actual host for the event
      const hostId = c.get('hostId');
      ...
      if (hostId && teamId) {
        ...
        // Use the team's host ID for the event
        actualHostId = hostId;
      }
      ...
      const transformedPlans = membership_plans.map((plan) => ({
        name: plan.name,
        user_id: actualHostId, // Use the actual host ID for memberships too
  ...
  public updateEvent = async (c: Context) => {
    try {
      ...
      // Get hostId from context and if hostId exists (team access)
      //If host id, check if current user has team access
      const hostId = c.get('hostId');
      const teamId = c.get('teamId');
      ...
      //only master role, admin, the owner of the event, or team members can update the event
      if (
        user.role !== 'master' &&
        user.role !== 'owner' &&
  ...
  public cancelEvent = async (c: Context) => {
    try {
      ...
      //only master role, admin, the owner of the event, or team members can cancel the event
      if (
        user.role !== 'master' &&
        user.role !== 'owner' &&
        event.host_id !== user.id &&
  ...
  public deleteEvent = async (c: Context) => {
    try {
      ...
      //only master role, admin, the owner of the event, or team members can delete the event
      if (
        user.role !== 'master' &&
        user.role !== 'owner' &&
        event.host_id !== user.id &&
  ...
  public streamPrerecordedEvent = async (c: Context) => {
    try {
      ...
      if (body.isHost) {
        const lobby_telemetry = await this.telemetryService.getLobbyTelemetryByEventId(
          event_id,
          false,
        );
        ...
        return c.json({ event, isHost: true, selectedDates: allDates, lobby_telemetry });
      }
      if (!token) {
      ...
      const membership = await this.membershipService.getMembership(lead.membership_id);
      if (!membership) {
        return serveBadRequest(c, ERRORS.MEMBERSHIP_NOT_FOUND);
      }

      // Find the matching membership in event.memberships
      const matchingMembership = event.memberships.find((m) => m.id === lead.membership_id) as
        | { dates: { id: number }[] }
        | undefined;

      // Map lead's selected dates to full date objects from the matching membership
      const selectedDates = (matchingMembership?.dates || []).find(
        (date: { id: number }) => date.id === lead.date_id,
      );
  ...
  /**
   * Gets server time for clock synchronization
   * @param {Context} c - The Hono context
   * @returns {Promise<Response>} Response containing server time
   */
  public getServerTime = async (c: Context) => {
    try {
      const serverTime = new Date().toISOString();
      const timestamp = Date.now();
      const eventId = c.req.param('eventId'); // Optional event-specific sync
      ...
      return c.json({
        ...
        latency: 0, // Will be calculated client-side
      });
    } catch (error) {
      logger.error(error);
      return serveInternalServerError(c, error);
