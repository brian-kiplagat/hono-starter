...
import { Event, User } from '../../schema/schema.ts';
...
import { sendTemplateEmail, sendTransactionalEmail } from '../../task/email-processor.ts';
...
export class StripeController {
  ...
  public createConnectAccount = async (c: Context) => {
    try {
      ...
      if (user.stripe_connect_id) {
        const account = await this.stripeService.getAccountStatus(user.stripe_connect_id);
  ...
  private handleSubscriptionUpdate = async (subscription: any) => {
    try {
      ...
      const { priceId, productId, product } = subscription.metadata;
      ...
      const productName = canceledStatuses.includes(subscription.status) ? 'free' : product;
  ...
  private handleTrialEnding = async (subscription: any) => {
    try {
      ...
      // Send welcome email
      sendTransactionalEmail(user.email, user.name, 12, MAIL_CONTENT.SUBSCRIPTION_TRIAL_ENDED);
    } catch (error) {
      logger.error(error);
      throw error;
    }
    ...
    try {
      ...
      if (type === 'lead_upgrade') {
        if (session.status === 'complete') {
          ...
          if (lead) {
            await this.leadService.update(lead.id, {
              ...
              membership_id: Number.parseInt(membershipId),
              date_id: date_ids[0],
              metadata: {
            ...
            const membershipDates = await this.membershipService.getMembershipDates(
              Number.parseInt(membershipId),
            );
            if (!membershipDates) {
              logger.error(`DATE ERROR - No DATES found for lead ${leadId} with dates ${date_ids}`);
              return;
            }
            const dates = membershipDates.map((date) => Number(date.date));
            ...
            const formattedDates = dates.map((date) =>
              formatDateToLocale(new Date(Number(date) * 1000), 'Europe/London'),
            );
            const eventDate = formatter.format(formattedDates);

            //check if host
            const host = await this.userService.find(Number(event?.host_id));
            const business = await this.businessService.getBusinessByUserId(host?.id || 0);
            ...
            if (lead.email && lead.name) {
              ...
              const attachment = await this.icsService.createAttachment(
                event as Event,
                lead,
                host as User,
                business,
              ...
              sendTemplateEmail(
                ...
                4,
                {
                  ...
                  eventdate: eventDate.split(',').slice(0, -1).join(',').trim(),
                  eventtime: eventDate.split(',').pop()?.trim() || '',
                  ...
                  calendarurl: event?.calendar_url || 'No link available currently',
                },
                attachment,
              );
            }
            ...
            await this.notificationService.create({
              user_id: event?.host_id || 0,
              notification_type: 'new_payment',
              link: `/concepts/event/event-edit/${event?.id}`,
              metadata: {
                lead_id: lead.id,
                situation_id: event?.id,
                situation_name: event?.event_name,
                lead_name: lead.name,
                lead_email: lead.email,
                dates: date_ids,
              },
              title: `New payment`,
              message: `${lead.name} has paid ${session.amount_total} GBP for "${event?.event_name}"`,
            });
          }
        }
        ...
        await Promise.all([
          ...
          // Send welcome email
          sendTransactionalEmail(user.email, user.name, 12, {
